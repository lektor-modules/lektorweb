import {Vector2,
	...
	CurvePath,
	CubicBezierCurve3,
	Raycaster } from "./libs.js"

export let mapgen = () => {

	const d=220, R=160, roAtmDeg=-52, T_earth='assets/mod/glomaps/map.png',
	...
	var Ematerial=new PointsMaterial({
		...
		depthTest: false,
		onBeforeCompile: sh=>{
			// console.log (sh) // enable for debug only
...
'			+sh.fragmentShader.replace("#include <map_particle_fragment>", `
 ...
 diffuseColor.a *= smoothstep( ${(R*.2).toFixed(1)},  ${(-R*.4).toFixed(1)}, fogDepth-${(posZ).toFixed(1)} )*size;
	...
	Tmaterial.size=.047*d; Tmaterial.opacity=.5;
...
//})()
	...
	// document.querySelector('.globes').classList.remove('prepare')
